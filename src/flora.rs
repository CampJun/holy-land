...
/// One cell of a 2.5D building's roof overlay. A roofed building is
/// stamped over its prefab footprint; each cell carries a `Roof` naming
/// the `Sheet::Structures` slice it draws (roof slope on the upper rows,
/// wall + window + door facade on the bottom row). The roof is a pure
/// render layer — walkability/LOS read terrain only, exactly as before.
///
/// Reveal: all cells of one building share an `anchor` (the footprint's
/// top-left world cell). When the player stands on an `is_interior` cell
/// of building A, A's whole roof is suppressed in render so the interior
/// (floor/walls/furniture under the roof) shows through. Footprints never
/// overlap, so this stays a per-cell pick with no z-ordering — like
/// `Canopy`. Regenerated by chunkgen every load; never persisted.
...
impl Roof {
    /// Whether this roof cell should be hidden given the building the
    /// player currently stands inside (its anchor, or `None` when the
    /// player is outdoors). A building hides exactly its own roof.
    pub fn hidden(self, active: Option<(i32, i32)>) -> bool {
        active == Some(self.anchor)
    }
}
...
mod tests {
    ...
    #[test]
    fn roof_hidden_only_for_the_building_youre_inside() {
        let r = Roof { anchor: (10, 20), col: 6, row: 1, is_interior: false };
        // Outdoors: no active building -> every roof draws.
        assert!(!r.hidden(None));
        // Inside this building -> its roof hides.
        assert!(r.hidden(Some((10, 20))));
        // Inside a *different* building -> this roof still draws.
        assert!(!r.hidden(Some((99, 99))));
    }
