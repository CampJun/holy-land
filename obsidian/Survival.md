...
- [ ] [[Survival - Species variety - trees and undergrowth]]
	Tree species (Oak/Hazel/Holly/Ash/Rowan) via `CellState.tree_species`; per-species seasonal tints replace `TREE_TINT_VARIANTS`. Undergrowth as `CellState.decoration` (Fern/Moss/Bramble/Bracken/Gorse/Sapling/Mushroom) with interactive harvest verbs. **Phase D shipped the user-visible half:** chunkgen places undergrowth on ~15% of non-spawn-disc Grass cells (shade-vs-open weighted: Fern/Moss favored near canopy, Gorse/Bracken/Bramble in the open). Five harvest verbs land — HarvestMoss/CutFern/DigFern (knife) + CutGorse (axe) + CutBracken (knife) — each finds its decoration in the 3x3 catchment, clears via `set_decoration_at`, drops yields into pack (or onto the cell if full), spends time, and awards Foraging XP where applicable. ChopTree now stamps `BareDirt + tree_species=None + Decoration::Sapling { species, planted_day }` so the chop→regrow loop is closed (regrowth in [[Survival - Plant lifecycle and seasonal foraging]]). Save round-trip uses sparse `tree_species_mutations` + `decoration_mutations` lists alongside the existing `terrain_mutations`. PickBrambleFruit + PickMushroom (the verbs that need a fruit/mushroom state from the lifecycle scheduler) stay parked until [[Survival - Plant lifecycle and seasonal foraging]] is fully fleshed out.
